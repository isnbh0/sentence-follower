...
const credentials =
  process.env.MODE === "local"
    ? JSON.parse(
        fs.readFileSync(path.join(__dirname, "../.secrets/chrome.json")),
      )
    : {
...
if (
  ...
  if (!credentials.chromeExtensionId) {
    console.error(
      "❌ GOOGLE_CHROME_EXTENSION_ID not found in environment variables",
    );
  }
  process.exit(1);
}
    ...
    try {
      ...
      if (responseData.uploadState === "FAILURE" && responseData.itemError) {
        ...
        if (error.error_code === "ITEM_NOT_UPDATABLE") {
          console.log(
            "❗ Extension update blocked: The item is currently in pending review, ready to publish, or deleted status",
          );
          console.log(
            "❗ Please wait for the review to complete or check the extension status in the Chrome Web Store dashboard",
          );
...
async function publishExtension(accessToken) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Publish failed with status ${response.status}: ${errorText}`,
      );
    }
...
async function publishChromeExtension() {
  ...
  try {
    ...
    console.log(
      "❗ NOTE: Publishing is to be done manually from the Chrome Web Store dashboard.",
    );
  } catch (error) {
    console.error(
      `❌ An error occurred during the publishing process: ${error.message}`,
    );
    process.exit(1);
  }
