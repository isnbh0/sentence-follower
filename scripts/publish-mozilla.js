...
const credentials =
  process.env.MODE === "local"
    ? JSON.parse(
        fs.readFileSync(path.join(__dirname, "../.secrets/mozilla.json")),
      )
    : {
...
try {
  ...
  execSync(`web-ext lint --config=${configPath}`, { stdio: "inherit" });
  ...
  if (stderr.includes("Approval: timeout exceeded")) {
    const urlMatch = stderr.match(
      /https:\/\/addons\.mozilla\.org.*\/versions\/\d+/,
    );
    if (urlMatch) {
      console.warn(
        `⚠️ Approval timed out. Signed XPI available at: ${urlMatch[0]}`,
      );
    } else {
      console.warn(
        "⚠️ Approval timed out. XPI URL not found in the error message.",
      );
    }
    process.exit(0);
  } else {
